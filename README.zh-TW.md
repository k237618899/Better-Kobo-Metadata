...
- 小數卷號處理：可正確保留 `13.5` 這類系列號，不再被截斷成錯誤卷數。
...
- 擴充書籍欄位解析：支援從 Kobo 詳細資料中抓取語言、書籍 ID、合法 ISBN 與 EAN 類型識別碼。
- calibre 友善評分匯入：擷取 Kobo 評分並依 0-5 分制（半星步進）正確寫入。
- 從標題回推系列名稱：Kobo 未提供系列區塊時的 fallback。
- CJK 空格正規化：移除擷取欄位中 CJK 字元之間的意外空格。
- 僅使用全球站：統一使用穩定的 `kobo.com`，不再提供 mirror 站切換。
